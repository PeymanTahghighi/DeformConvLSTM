...
class DeformableConvLayer2D(tf.keras.layers.Layer):
    ...
    def __init__(self,
        ...
        # tf_shapeHidden = tf.TensorShape([20,10] + [10])
        # tf_shapeState = tf.TensorShape([20,10] + [10])
        ...
        self.offsetInitializer = tf.zeros_initializer();
        self.xOffset = None;
        self.yOffset = None;
        if(bias_initializer is None):
            self.bias_initializer = tf.random_normal_initializer(0.0,0.0);
        else:
    ...
    def call(self, inputs):
        ...
        # get x, y axis offset
        offset = tf.reshape(offset, [batch_size, out_h, out_w, self.kernel_size[0] * self.kernel_size[1] * self.num_deformable_group, 2])
        ...
        p0, p1, p2, p3 = [DeformableConvLayer2D._get_pixel_values_at_point(inputs, i) for i in indices]
        # cast to float
        ...
        # depth-wise conv
        out = tf.nn.conv2d(pixels, self.kernel, [1, filter_h, filter_w, 1], 'VALID')
